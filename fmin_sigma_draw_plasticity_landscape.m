...
Y1=Y(in==1);
X1=X(in==1);
%% The approximation of phenotypic landscape without BRD
% mean
...
%% The approximation of phenotypic landscape with BRD
% mean
...
% The phenotypic landscape without BRD
...
% The phenotypic landscape with BRD
...
% The plasticity landscape
Z = Z1 - Z2;

% triangulation to draw plasticity landscape
T= delaunay(X1, Y1);
% Extracts the vertex index of the triangle
points = [X1,Y1];

% Extracting triangle vertices
triPoints = points(T, :);
% Calculate the sides of the triangle
edges = [T(:,[1 2]); T(:,[2 3]); T(:,[3 1])];
edgeLengths = sqrt(sum((triPoints(edges(:,1),:) - triPoints(edges(:,2),:)).^2, 2));

% Set the edge length threshold
edgeLengthThreshold = 0.3;

validTri = [];

% Go through each triangle
for i = 1:size(T, 1)
    % Extract the side length of the triangle
    ...
    % If all sides are less than the threshold, the triangle is preserved
    if dis < edgeLengthThreshold
        validTri = [validTri; T(i,:)];
    end
end

% Optional Re-triangulate the remaining points
% If you have removed many triangles, you may want to re-triangulate the remaining set of points to get a more reasonable split
% Note This step may not be necessary, depending on your application requirements

% remainingPoints = points(unique(validTri), :);
% reTri = delaunay(remainingPoints(:,1), remainingPoints(:,2));

% plot plasticity landscape
figure;
